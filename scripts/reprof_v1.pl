...
my @model_names = qw(a u b uu ub bu bb fa fu fb fuu fub fbu fbb);
my %parsers;
my %models;
my %features;
my @mutations;
...
my %ori_fasta_predictions;  # needed if mutations are provided to save time calculating inputs
...
if (exists $parsers{blastPsiMat}) {
    ...
    $outputs{uu} = run_model($models{uu}, create_inputs($features{uu}->[0], $u_out));
    $outputs{ub} = run_model($models{ub}, create_inputs($features{ub}->[0], $u_out));
    $outputs{bu} = run_model($models{bu}, create_inputs($features{bu}->[0], $b_out));
    $outputs{bb} = run_model($models{bb}, create_inputs($features{bb}->[0], $b_out));

    my $sec = jury($uu_out, $ub_out, $bu_out, $bb_out);
    write_output($parsers{fasta}, $out_file, $sec, $a_out);
}
if (! exists $parsers{blastPsiMat} || defined $mutation_file) {
    $outputs{fa} = run_model($models{fa}, create_inputs($features{fa}->[0]));
    $outputs{fu} = run_model($models{fu}, create_inputs($features{fu}->[0]));
    $outputs{fb} = run_model($models{fb}, create_inputs($features{fb}->[0]));

    $outputs{fuu} = run_model($models{fuu}, create_inputs($features{fuu}->[0], $outputs{fu}));
    $outputs{fub} = run_model($models{fub}, create_inputs($features{fub}->[0], $outputs{fu}));
    $outputs{fbu} = run_model($models{fbu}, create_inputs($features{fbu}->[0], $outputs{fb}));
    $outputs{fbb} = run_model($models{fbb}, create_inputs($features{fbb}->[0], $outputs{fb}));
...
if (defined $mutation_file) {
    ...
    #$ori_fasta_predictions = predict_fasta();
    #write_output($parsers{fasta}, "$out_file\_ORI", @$ori_fasta_predictions);
}
...
foreach my $mutation (@mutations) {
    ...
    #$predictions = predict(($i - 1), $ori_fasta_predictions);

    #write_output($parsers{fasta}, "$out_file\_$aa_ori$i$aa_mut", @$predictions);
}

# Subs
sub write_output {
    my ($fasta_parser, $out_file, $sec_prediction, $acc_prediction) = @_;
